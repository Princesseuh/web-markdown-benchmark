...
export async function runAllBenchmarks(onProgress: ProgressFn = () => {}): Promise<BenchResult[]> {
  ...
  for (const scenario of markdownPluginScenarios) {
    onProgress(`\n${scenario.name} (${SMALL}×)`);
    await record("md-plugin", scenario.name, "satteri", SMALL, () => scenario.satteri(medium));
